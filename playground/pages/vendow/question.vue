...
async function updateRead(nextVal: boolean) {
  ...
  try {
    await request(`consultations/${activeItem.value.id}`, {
      method: "post",
      data: { read: nextVal },
    });
    $notify(
...
<template>
  <NuxtLayout name="admin">
    <template #main>
      ...
      >
        <Header>سوال‌های مشتریان</Header>
        ...
        <template #list-item="{ item }">
          ...
          <div class="w-full">
            <div class="flex items-center gap-2 mb-2">
              <h2 class="font-semibold text-lg">{{ item.fullname }}</h2>
              <h2 class="font-semibold text-lg">{{ item.phone }}</h2>
            </div>
            <p class="text-sm">
