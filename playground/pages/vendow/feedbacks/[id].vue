...
async function fetData() {
  ...
  if (data.value.status) {
    ...
    products.value = { name: c.productName, id: c.productId };
...
const approve = async (flag) => {
  await request(`productcomments/${useRoute().params.id}/${flag}`, {
    method: "patch",
    data: {},
  });
  useNuxtApp().$notify(
...
// submit
const submit = async () => {
  await request(`productcomments/${useRoute().params.id}`, {
    method: "post",
