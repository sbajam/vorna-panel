...
export default eventHandler(async (event) => {
  ...
  const file = formData?.find(part => part.name === 'image' && part.data?.length > 0)
  const title = formData?.find(part => part.name === 'title')?.data?.toString() || ''

  if (!file || !file.data) {
    console.log('File not found in request. Available parts:', formData?.map(p => p.name))
    throw createError({ 
      statusCode: 400, 
  ...
  } catch { /* فایل ممکن است هنوز نباشد */ }
