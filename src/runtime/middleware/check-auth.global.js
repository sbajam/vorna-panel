...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  const isSuperAdmin =
    config.superAdmins?.includes(userRole) ||
  ...
  try {
    const permissionsRes = await request(
      `/api/user-permissions?role=${userRole}`,
